...
<body class="gradient-bg">
    ...
    @if(isset($isKesiswaan) && $isKesiswaan)
        @include('layouts.kesiswaan-navbar', ['title' => 'Kelola Siswa', 'subtitle' => 'Data Siswa SMK Bakti Nusantara 666'])
    @elseif(isset($isKepsek) && $isKepsek)
        @include('layouts.kepsek-navbar', ['title' => 'Data Siswa', 'subtitle' => 'Data Siswa SMK Bakti Nusantara 666'])
    @else
                ...
                <form id="kelolaForm" method="POST" action="{{ route('admin.kelola-siswa') }}" enctype="multipart/form-data">
                    ...
                    <div id="prestasiSection" class="mb-6 hidden">
                        <div class="mb-4">
                            ...
                            <select id="kategori_prestasi" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" onchange="filterJenisPrestasi()">
                                <option value="">Pilih Kategori</option>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="prestasi_id" id="jenis_prestasi_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" disabled>
                                <option value="">Pilih kategori terlebih dahulu</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div id="buktiGambarSection" class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Bukti Gambar <span class="text-gray-500">(Opsional)</span>
                        </label>
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                            <input type="file" name="bukti_gambar" id="bukti_gambar" accept="image/*" class="hidden">
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div class="p-6">
                ...
                <div id="detailContent" class="hidden">
                    ...
                    <!-- Data Orang Tua -->
                    <div id="orangTuaProfile" class="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl p-6 mb-6 border border-yellow-200">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-lg font-bold text-gray-800">Data Orang Tua</h4>
                            <span id="statusAkunOrangTua" class="px-3 py-1 text-sm bg-orange-100 text-orange-800 rounded-full font-medium">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <span id="profileNamaAyah" class="font-medium text-gray-800 ml-2">-</span>
                                    </div>
                                </div>
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <span id="profileNamaIbu" class="font-medium text-gray-800 ml-2">-</span>
                                    </div>
                                </div>
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <span id="profileNoHp" class="font-medium text-gray-800 ml-2">-</span>
                                    </div>
                                </div>
                            </div>
                            <div class="space-y-3">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <span id="profilePekerjaanAyah" class="font-medium text-gray-800 ml-2">-</span>
                                    </div>
                                </div>
                                <div class="flex items-center">
                                    <i class="fas fa-briefcase text-yellow-600 w-5 mr-3"></i>
                                    <div>
                                        ...
                                        <span id="profilePekerjaanIbu" class="font-medium text-gray-800 ml-2">-</span>
                                    </div>
                                </div>
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <span class="text-sm text-gray-600">Alamat:</span>
                                        <span id="profileAlamatOrtu" class="font-medium text-gray-800 ml-2">-</span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.getElementById('showMoreBtn')?.addEventListener('click', function() {
            ...
            fetch(`{{ isset($isKesiswaan) && $isKesiswaan ? '/kesiswaan' : '/admin' }}/load-more-siswa?${params}`)
        ...
        function openDetailModal(siswaId, nama, nis, kelas) {
            ...
            fetch(`{{ isset($isKesiswaan) && $isKesiswaan ? '/kesiswaan' : '/admin' }}/get-orang-tua/${siswaId}`)
                .then(response => response.json())
                .then(siswaData => {
                    // Load pelanggaran data
                    return fetch(`{{ isset($isKesiswaan) && $isKesiswaan ? '/kesiswaan' : '/admin' }}/siswa/${siswaId}/pelanggaran`)
                        .then(response => response.json())
                        .then(pelanggaranData => {
                            return { siswa: siswaData, pelanggaran: pelanggaranData.pelanggaran };
                        });
                })
                .then(data => {
                    ...
                    // Populate orang tua data
                    if (siswa.orang_tua) {
                        document.getElementById('profileNamaAyah').textContent = siswa.orang_tua.nama_ayah || '-';
                        document.getElementById('profileNamaIbu').textContent = siswa.orang_tua.nama_ibu || '-';
                        document.getElementById('profileNoHp').textContent = siswa.orang_tua.no_hp || '-';
                        document.getElementById('profilePekerjaanAyah').textContent = siswa.orang_tua.pekerjaan_ayah || '-';
                        document.getElementById('profilePekerjaanIbu').textContent = siswa.orang_tua.pekerjaan_ibu || '-';
                        document.getElementById('profileAlamatOrtu').textContent = siswa.orang_tua.alamat || '-';
                        
                        // Update status akun orang tua
                        const statusAkunOrangTua = document.getElementById('statusAkunOrangTua');
                        if (siswa.orang_tua_user_id) {
                            statusAkunOrangTua.className = 'px-3 py-1 text-sm bg-green-100 text-green-800 rounded-full font-medium';
                            statusAkunOrangTua.innerHTML = '<i class="fas fa-users-check mr-1"></i>Akun Terdaftar';
                        } else {
                            statusAkunOrangTua.className = 'px-3 py-1 text-sm bg-orange-100 text-orange-800 rounded-full font-medium';
                            statusAkunOrangTua.innerHTML = '<i class="fas fa-user-clock mr-1"></i>Belum Terdaftar';
                        }
                    } else {
                        document.getElementById('profileNamaAyah').textContent = '-';
                        document.getElementById('profileNamaIbu').textContent = '-';
                        document.getElementById('profileNoHp').textContent = '-';
                        document.getElementById('profilePekerjaanAyah').textContent = '-';
                        document.getElementById('profilePekerjaanIbu').textContent = '-';
                        document.getElementById('profileAlamatOrtu').textContent = '-';
                        document.getElementById('statusAkunOrangTua').style.display = 'none';
                    }
        ...
        // Filter jenis prestasi by category
        const allPrestasi = @json(App\Models\Prestasi::orderBy('kategori')->orderBy('poin_pengurangan', 'desc')->get());
        
        function filterJenisPrestasi() {
            ...
            if (kategori) {
                jenisSelect.disabled = false;
                const filteredPrestasi = allPrestasi.filter(p => p.kategori.toLowerCase() === kategori.toLowerCase());
